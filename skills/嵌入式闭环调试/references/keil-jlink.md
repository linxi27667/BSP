...
某些 µVision 版本在成功构建时 `$LASTEXITCODE` 为空或不可靠。读取 `build.log` 的最终汇总，要求零错误；高可靠项目同时要求零警告，并确认 HEX/AXF 的修改时间属于本轮构建。不要因退出码异常否定一份明确成功的日志，也不要因退出码为零忽略日志错误。
...
- 架空只是卸载测试，不能证明车辆落地后的摩擦、偏载和电池压降性能。
- 固定流程应覆盖正转、反转、多个速度档、停止残留和至少两轮重复性；推荐 5 轮。
- 每档记录目标、左右反馈、左右输出、稳态误差和通过标志；跨轮计算变异系数。
- 固件内门槛负责即时失败，主机脚本负责检查缺失阶段、终态、重复性并生成 JSON/Markdown 报告。
- 台架通过的严谨措辞是“具备受控低速落地直线测试资格”，不是“已经可以稳定循迹”。
